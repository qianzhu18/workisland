...
test("every Settings Agent maps to a bundled brand asset", () => {
  ...
  for (const [agentId, filename] of Object.entries(EXPECTED_AGENT_ICONS)) {
    ...
    const property = /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(agentId) ? agentId : `"${agentId}"`;
    assert.match(settingsSource, new RegExp(`${property}: "\\.\\.\\/assets\\/brands\\/${escapedFilename}"`));
