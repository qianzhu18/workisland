...
## Now: v3.0.0 Public Pilot

目标：确认真实多 Agent 用户能完成“看得见、叫得回、接得上”的闭环。

- `v3.0.0`：公开测试前先完成核心会话闭环、可审计的匿名统计与操作手册；统计默认开启、设置内披露并可随时关闭。
- 只围绕最高频阻断问题决定 `v3.0.x` 修复版本；每个修复版本只验证一个主假设。
- 历史 Beta 保持预发布；`v3.0.0` 是首个进入稳定更新通道的公开版本。
