...
以上命令只用于本机测试未签名构建；正式签名和公证包应优先按系统提示直接打开。若下载后的 DMG 本身被隔离，也可以先对 DMG 执行 `xattr -c "/path/to/WorkIsland-0.2.5-arm64.dmg"`，再重新挂载。
...
产物位于 `release/WorkIsland-<version>-arm64.dmg`。GitHub Actions 的 `release` 工作流支持手动运行；推送与 `package.json` 版本一致的标签（例如 `v0.2.5`）时，会自动创建 GitHub Release 并上传 DMG 与 SHA-256 校验文件。
