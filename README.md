...
- **让智能体安全操作产品**——可选开启本地 MCP，让 Codex 读取和修改白名单设置、聚焦脱敏会话；每次修改可见、可撤销，默认不授权。

## 智能体控制（MCP）

在「设置 → 智能体控制」中开启授权并一键连接 Codex。WorkIsland 只开放显式白名单能力，不提供任意命令执行、审批代办或会话内容读取；关闭开关后下一次调用立即失效。配置方法、工具清单、CLI、隐私边界和移除步骤见 [本地智能体控制说明](docs/local-agent-control.md)。
