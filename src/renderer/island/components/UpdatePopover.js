...
export function UpdateStatusButton({ updateState, hasUpdate, onDownload, onInstall, onOpenRelease }) {
  ...
  const primaryAction = (() => {
    if (displayPhase === "ready") return { label: t("update.action.install"), onClick: onInstall };
    if (displayPhase === "downloading" || displayPhase === "installing") return null;
    return { label: t(displayPhase === "error" ? "update.action.retry" : "update.action.download"), onClick: onDownload };
  })();
  const statusText = (() => {
    if (displayPhase === "downloading") return t("update.status.downloading");
    if (displayPhase === "installing") return t("update.status.installing");
    if (displayPhase === "manual") return t("update.status.manual");
    if (displayPhase === "error") return updateState?.error || t("update.status.error");
    return null;
  })();
  const headerBadge = (() => {
    if (displayPhase === "ready") return t("update.badge.ready");
    if (displayPhase === "downloading") return t("update.badge.downloading");
    if (displayPhase === "installing") return t("update.badge.installing");
    if (displayPhase === "manual") return t("update.badge.manual");
    if (displayPhase === "error") return t("update.badge.error");
    return t("update.badge.available");
  })();
  const releaseVersion = updateState?.latestVersion;
  const popover = visible && React.createElement("div", { ref: popoverRef, className: "update-popover", role: "dialog", "aria-label": t("update.title"), style: { left: `${position.left}px`, top: `${position.top}px` } },
    React.createElement("div", { className: "update-popover-header" },
      React.createElement("strong", null, releaseVersion ? `WorkIsland ${releaseVersion}` : t("update.workIsland")),
      React.createElement("span", null, headerBadge)
    ),
    React.createElement("p", { className: "update-popover-desc" }, t("update.description")),
    ...
    React.createElement("div", { className: "update-popover-actions" },
      ...
      releaseVersion && onOpenRelease && React.createElement("button", { type: "button", className: "update-popover-link", onClick: onOpenRelease }, t("update.action.releaseNotes"))
    )
  );
  return React.createElement("div", { ref: triggerRef, className: "usage-cell update-cell" },
    React.createElement("button", { type: "button", className: `update-tag is-${displayPhase}`, onClick: () => setPinned((value) => !value), "aria-expanded": visible, title: releaseVersion ? t("update.availableVersion", { version: releaseVersion }) : t("update.available"), "aria-label": t("update.title") },
