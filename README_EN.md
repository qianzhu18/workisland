...
**For users:** download the Apple Silicon DMG from [GitHub Releases](https://github.com/qianzhu18/workisland/releases). No cloud account is required.

> **Versioning**: 0.2.x was the private beta line; 1.x is the public stable line. WorkIsland was rewritten in 2026-08 building on two earlier prototypes.
>
> **Windows**: Windows support is on hold (code, CI, and build scripts are fully retained). We do not ship public Windows builds until they meet our quality bar. If you use Windows daily and want to lead the port, please [open an issue](https://github.com/qianzhu18/workisland/issues/new/choose).

**For contributors:** WorkIsland requires an Apple Silicon Mac (Windows 11 x64 works for contributors on the Windows port) and Node.js 22 or later.
...
Contributors on the Windows port can use `npm run package:win` to build the installer and portable EXE (not published as public artifacts for now).
...
- **Return to the right source session** - jump back to the matching Terminal, iTerm2, Ghostty, Warp, Cursor, or VS Code conversation.
- **Use the local workstation** - media and lyrics, process performance, Shelf, private clipboard history, and a persistent terminal.
